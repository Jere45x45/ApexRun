...
public static class KartBuilder
{
    public static KartStats Build(RuntimeKartConfiguration configuration)
    {
        ...
        if (configuration.Engine == null)
            throw new InvalidOperationException("El kart no tiene un motor instalado.");

        if (configuration.Chassis == null)
            throw new InvalidOperationException("El kart no tiene un chasis instalado.");

        if (configuration.Wheels == null)
            throw new InvalidOperationException("El kart no tiene ruedas instaladas.");
